  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="h-2 overflow-hidden rounded-full bg-neutral-200">
        <div
          className={`h-full rounded-full transition-[width] duration-500 ${isAtLimit ? 'bg-red-500' : 'bg-neutral-900'}`}
          style={{ width: `${pct}%` }}
        />
      </div>
      {isAtLimit && (
        <div className="mt-2 flex items-center justify-between rounded-lg border border-amber-200 bg-gradient-to-r from-orange-50 to-rose-50 px-3 py-2.5">
          <div>
            <p className="text-xs font-semibold text-neutral-900">{t.freeLimit}</p>
            <p className="mt-0.5 text-xs text-neutral-500">{t.upgradeToHelp}</p>
          </div>
          <Link
            href="/pricing"
